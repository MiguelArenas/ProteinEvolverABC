# R script to apply priors and make executable files "ProteinEvolverABC_Phase3.sh" and "ProteinEvolverABC_arguments.txt" for ProteinEvolverProtABC2.0.0
# By Miguel Arenas
# 2020
# 
...
unlink ("./ProteinEvolverABC_Phase3.sh", recursive = FALSE)
##### GENERAL SETTINGS #####
NumberReplicates <- 5000
...
while (ThisReplicate < NumberReplicates)
	...
	SampleSize_SequenceLength_print <- paste (" -s11 289",sep="")
	...
	HomogRec <- runif(1,0,6.92e-5)		# fix, unif, norm(t), exp(t), gamma(t), beta(t)
	HomogRec_print <- paste (" -r",HomogRec,sep="")
	recombination_rate[ThisReplicate]<-HomogRec
	SubsRate <- runif(1,0,2.5952e-4)		# fix, unif, norm(t), exp(t), gamma(t), beta(t)
	...
	SubsModel_print <- paste (" -@","UserEAAMsites",sep="")
	AArateHetSites_print <- paste ("",sep="")
	AAPinv_print <- paste ("",sep="")


    ExecutionHeader <- paste ("-n1",SampleSize_SequenceLength_print,PopSize_Hap_Dip_print,DatedTips_print,GenTime_print,GrowthRate_print,DemogPeriods_print,DemogPeriodsNcte_print,DemogPeriodsNvar_print,Migrationmodel_print,MigrationRate_print,ConvDemes_print,HomogRec_print,SubsRate_print,outgroup_print,AminoacidFrequencies_print,SubsModel_print,AArateHetSites_print,AAPinv_print," -bsequences -xseqGMRCA -c1 1 0 -y0 -:",ThisReplicate,sep="")

	write(paste("",ExecutionHeader,"",sep=""),"./ProteinEvolverABC_arguments.txt",append=T)
	Rho_sim <- 2 * 2 * PopSize * HomogRec * 289 
	HomogRec_print_Pop <- paste (" -r",Rho_sim,sep="")

	Theta_sim <- 2 * 2 * PopSize * SubsRate * 289 
	SubsRate_print_pop <- paste (" -u",Theta_sim,sep="")
	RHO[ThisReplicate]<-Rho_sim
	THETA[ThisReplicate]<-Theta_sim
    ExecutionHeader_Pop <- paste ("-n1",SampleSize_SequenceLength_print,PopSize_Hap_Dip_print,DatedTips_print,GenTime_print,GrowthRate_print,DemogPeriods_print,DemogPeriodsNcte_print,DemogPeriodsNvar_print,Migrationmodel_print,MigrationRate_print,ConvDemes_print,HomogRec_print_Pop,SubsRate_print_pop,outgroup_print,AminoacidFrequencies_print,SubsModel_print,AArateHetSites_print,AAPinv_print," -bsequences -xseqGMRCA -c1 1 0 -y0 -:",ThisReplicate,sep="")

	write(paste("",ExecutionHeader_Pop,"",sep=""),"./ProteinEvolverABC_Pop_arguments.txt",append=T)

} # end replicates
	NumberOfProcessors <- 1
	NumberOfArguments <- 36
	bodyhereNew<-paste("cat \"./ProteinEvolverABC_arguments.txt\" | xargs -n ",NumberOfArguments," \"./bin/ProteinEvolverProtABC2.0.0\"",sep="")
	write(paste("",bodyhereNew,"",sep=""),"./ProteinEvolverABC_Phase3.sh",append=T)
