...
while (ThisReplicate < NumberReplicates)
	...
	SampleSize_SequenceLength_print <- paste (" -s133 108",sep="")
	...
	HomogRec <- runif(1,0,9.92e-5)		# fix, unif, norm(t), exp(t), gamma(t), beta(t)
	HomogRec_print <- paste (" -r",HomogRec,sep="")
	recombination_rate[ThisReplicate]<-HomogRec
	SubsRate <- runif(1,0,5.5952e-4)		# fix, unif, norm(t), exp(t), gamma(t), beta(t)
	...
	Rho_sim <- 2 * 2 * PopSize * HomogRec * 108 
	HomogRec_print_Pop <- paste (" -r",Rho_sim,sep="")

	Theta_sim <- 2 * 2 * PopSize * SubsRate * 108 
