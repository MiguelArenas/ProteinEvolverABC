...
print "\n****************************************************************************************************************";
print "\nProteinEvolverABC_Phase5.pl";
print "\nProteinEvolverABC Phase 5 does:";
print "\n- Read Settings file";
...
my $Subs_init = "";
my $Subs_dip = "default";
my $SSCPE = 0; # If 0, SSCPE is not run. If 1, SSCPE runs // ProtASR2
my $PDB_init = "";
my $PDB_dip = "default";
my $Chain_init = "";
...
while (<FROM>)  {
	...
	if ($_ =~ /^\*SummaryStatistics=/)  {
		...
		if (scalar grep( $_ < 1 || $_ > 20, @summaryStatistics) > 0) {
			print "\n\nERROR!. SummaryStatistics must be integers between 1 and 20 ($abcHCorr)\n\n";
			print "Type CTRL+C to abort the execution \n";
			my $HereError = <STDIN>;
			chop($HereError);
			exit;
...
#my $numberSummStats = 0;
    ...
    #$numberSummStats = 20;
    #}
my $numberSummStats = scalar(@summaryStatistics);
my $numberParams = 2;
...
print FILE_1 "nsstats <- length(sstats.set)\n";
...
# New section to consider a selection of SS
print FILE_1 "# building input files with the desired number of SS \n";
print FILE_1 "#cols_SSsims <- paste(sstats.char, collapse = ",") \n";
print FILE_1 "#cmd_SSsims <- paste0(\"cut -d\",\" -f\", cols_SSsims,\" SSsimulations.csv > SSsimulationsSelected.csv\") \n";
print FILE_1 "#system(cmd_SSsims)\n";

print FILE_1 "cols_SSsims <- paste(sstats.set, collapse=\",\") # the selected SS \n";
print FILE_1 "system2(\"cut\", args = c(\"-d,\", paste0(\"-f\", cols_SSsims), \"SSsimulations.csv\"), stdout = \"SSsimulationsSelected.csv\") \n";
print FILE_1 "sstats.fname <- \"./SSsimulationsSelected.csv\" \n";

print FILE_1 "# Again, the selected SS \n";
print FILE_1 "cols_SSsims <- sstats.set \n";
print FILE_1 "linea <- readLines(\"SSRealData.ss\", warn = FALSE) \n";
print FILE_1 "linea <- linea[1]  # only the first line \n";
print FILE_1 "tokens <- unlist(strsplit(linea, \"[\\t ]+\")) \n";
print FILE_1 "tokens <- tokens[tokens != \"\"]  # remove empty sites \n";
print FILE_1 "claves  <- gsub(\":\", \"\", tokens[seq(1, length(tokens), by = 2)]) \n";
print FILE_1 "valores <- tokens[seq(2, length(tokens), by = 2)] \n";
print FILE_1 "cols_SSsims <- cols_SSsims[cols_SSsims <= length(claves)] \n";
print FILE_1 "claves_sel  <- claves[cols_SSsims] \n";
print FILE_1 "valores_sel <- valores[cols_SSsims] \n";
print FILE_1 "linea_final <- paste0(claves_sel, \": \", valores_sel, collapse = \"\\t\") \n";
print FILE_1 "writeLines(linea_final, \"SSRealDataSelected.ss\") \n";
print FILE_1 "target.fname <- \"./SSRealDataSelected.ss\" \n";
...
print FILE_1 "# sstats<-sstats[,sstats.set] # as we filtered above, this is not needed now \n";
print FILE_1 "# target<-target[sstats.set] # as we filtered above, this is not needed now \n\n";
...
print FILE_1 "    tab.reg <- suppressWarnings(summary(abc.obj,unadj=FALSE,intvl=0.95,print=FALSE)) \n";
...
print FILE_1 "        posterior.d <- suppressWarnings(density(abc.obj\$adj.values[,i],weight=abc.obj\$weights/sum(abc.obj\$weights),from=params.min[i],to=params.max[i])) \n";
print FILE_1 "        lines(posterior.d,xlim=c(params.min[i],params.max[i]),col=\"blue\")\n";
print FILE_1 "        prior.d <- density(params[1:niter,i],from=params.min[i],to=params.max[i])\n";
print FILE_1 "        lines(prior.d,xlim=c(params.min[i],params.max[i]),col=\"black\")\n";
print FILE_1 "        if(!multipage)\n";
...
system ("mv $setsdir/SSsimulationsSelected.csv \"$setsdir/OtherOutputs\"");
system ("mv $setsdir/SSRealDataSelected.ss \"$setsdir/OtherOutputs\"");
