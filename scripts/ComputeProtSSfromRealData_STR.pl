...
# DAVID system ("perl \"$scrtdir/PDBseq_real_data.pl\" \"$setsdir/$NameOfPhylipFile\" \"$setsdir/$SSSeqPDB_dip\" ");

##############################################
##############################################
# Compute SS // need to pass the input info for the SS (aln, pdb, chain)
print "> Calculating summary statistics from the target data set (STR) ..\n";
system ("perl \"$scrtdir/SummS_from_AA_REALdata.2_STR.pl\" \"$setsdir/$NameOfPhylipFile\" \"$show\" \"$SSPDB_dip\" \"$SSChain_dip\" \"$SSSeqPDB_dip\" ");
