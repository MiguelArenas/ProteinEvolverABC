...
##############################################
##############################################
# Obtain input file without PDB sequence -> Now it is in another input file
#system ("perl \"$scrtdir/Clean_real_data.pl\" \"$setsdir/$file\" ");
system ("perl \"$scrtdir/PDBseq_real_data.pl\" \"$setsdir/$NameOfPhylipFile\" \"$setsdir/$SSSeqPDB_dip\" ");


##############################################
##############################################
# Compute SS
print "> Calculating summary statistics from the target data set (with STR SS) ..\n";
# Compute SS // need to pass the input info for the SS (aln, pdb, chain)
# DAVID
# system ("perl \"$scrtdir/SummS_from_AA_REALdata_NoIndels.2_STR.pl\" \"$setsdir/$NameOfPhylipFile\" \"$show\" \"$SSPDB_dip\" \"$SSChain_dip\" \"$SSSeqPDB_dip\" ");
system ("mv \"$setsdir/SS$NameOfPhylipFile.ss\" \"$setsdir/SSRealData.ss\"");


print "\nSummary statistics for the target data were successfully computed!\n";
exit;
