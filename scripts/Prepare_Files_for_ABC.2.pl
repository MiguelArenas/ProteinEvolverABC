...
my $numberSummStats = $NumberOfSS; #elena
my $numberParams = 2;
...
if ($numberSummStats == 20)  { #elena
	print $sstats_out "Phi,NSS,ChiSq,p_av,p_sd,p_sk,p_ku,H_av,H_sd,H_sk,H_ku,S,si_av,si_sd,si_sk,si_ku,av_DG,sd_DG,sk_DG,ku_DG\n"; #elena
}
elsif ($numberSummStats == 16)  { #elena
    print $sstats_out "Phi,NSS,ChiSq,p_av,p_sd,p_sk,p_ku,H_av,H_sd,H_sk,H_ku,S,si_av,si_sd,si_sk,si_ku\n"; #elena
}
else  {
	print "\n\nERROR!!!. The number of parameters must be 16 or 20 ($numberSummStats)\n";
	print "Type CTRL+C to abort the execution \n";
	my $HereError = <STDIN>;
	chop($HereError);
	exit;
...
for (my $i = 0; $i < $numberSummStats; $i++)  { #elena
    $sstatsAux[$i] = $i*2+1;
}
