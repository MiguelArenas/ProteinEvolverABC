### ComputeSSfromSimulatedData.pl, vs 1.0
### Miguel Arenas. 2020.
###
...
my $show = $ARGV[2];
my $SSPDB_dip = $ARGV[3];
my $SSChain_dip = $ARGV[4];
my $SSSeqPDB_dip = $ARGV[5];
...
#system ("perl \"$scrtdir/SummS_from_AA_Data.2.pl\" \"$file\" \"$show\"");
system ("perl \"$scrtdir/SummS_from_AA_SIMdata.2_STR.pl\" \"$file\" \"$show\" \"$SSPDB_dip\" \"$SSChain_dip\" \"$SSSeqPDB_dip\" ");
