###
### ProteinEvolverABC_Phase1.pl, vs 2.3
###
...
print FILE_1 "# R script to apply priors and make executable files \"ProteinEvolverABC_Phase3.sh\" and \"ProteinEvolverABC_arguments.txt\" for ProteinEvolverProtABC2.0.0\n";
...
print FILE_1 "unlink (\"$setsdir/ProteinEvolverABC_Phase3.sh\", recursive = FALSE)\n";
...
while (<FROM>) 
    ...
    # PDB file, required for SCS models (SSCPE, ProtASR2_WT, ProtASR2_MF)
        ...
        if ($_ =~ /^pdbsequence=/)
            {
                ...
                $NumberOfArguments = $NumberOfArguments + 1; # add arguments seqMRCA (Miguel)
        ...
        # Sequence of the PDB file, required for SCS models, for SS
        if ($_ =~ /^SSpdbsequence=/)
            {
...
$NumberOfArguments = $NumberOfArguments + 1; # add arguments -:
if (($Subs_dip eq "SSCPE") || ($Subs_dip eq "ProtASR2_WT") || ($Subs_dip eq "ProtASR2_MF"))
    {
    $NumberOfArguments = $NumberOfArguments + 1; # add arguments -x
    }
...
$counterLines = 0;
print "\n\n> Writing executable output file for simulations \"$setsdir/ProteinEvolverABC_Phase3.sh\", \"$setsdir/ProteinEvolverABC_arguments.txt\" and \"$setsdir/ProteinEvolverABC_Pop_arguments.txt\" ... ";
...
if ($show_info == 1)
    ...
    if (($SSCPE == 1) || ($ProtASR2 == 1))
        {
        print FILE_1 "    ExecutionHeader <- paste (\"-n1\",SampleSize_SequenceLength_print,PopSize_Hap_Dip_print,DatedTips_print,GenTime_print,GrowthRate_print,DemogPeriods_print,DemogPeriodsNcte_print,DemogPeriodsNvar_print,Migrationmodel_print,MigrationRate_print,ConvDemes_print,HomogRec_print,SubsRate_print,outgroup_print,AminoacidFrequencies_print,SubsModel_print,AArateHetSites_print,AAPinv_print,\" -bsequences -x$SeqPDB_dip -c1 1 0 -y1 -:\",ThisReplicate,sep=\"\")\n";
        }
    else
        {
    ...
    if (($SSCPE == 1) || ($ProtASR2 == 1))
        {
        print FILE_1 "    ExecutionHeader <- paste (\"-n1\",SampleSize_SequenceLength_print,PopSize_Hap_Dip_print,DatedTips_print,GenTime_print,GrowthRate_print,DemogPeriods_print,DemogPeriodsNcte_print,DemogPeriodsNvar_print,Migrationmodel_print,MigrationRate_print,ConvDemes_print,HomogRec_print,SubsRate_print,outgroup_print,AminoacidFrequencies_print,SubsModel_print,AArateHetSites_print,AAPinv_print,\" -bsequences -x$SeqPDB_dip -c1 1 0 -y0 -:\",ThisReplicate,sep=\"\")\n";
        }
    else
        {
...
##############################################
##############################################
# print ProteinEvolverABC_Phase3.sh
...
# ELENA #
	...
	#print FILE_1 "	bodyhereNew<-paste(\"more \\\"$setsdir/ProteinEvolverABC_arguments.txt\\\" | xargs -n \",NumberOfArguments,\" -P \",NumberOfProcessors,\" \\\"$maindir/bin/ProteinEvolverProtABC2.0.0\\\"\",sep=\"\")\n";
	print FILE_1 "	bodyhereNew<-paste(\"cat \\\"$setsdir/ProteinEvolverABC_arguments.txt\\\" | xargs -n \",NumberOfArguments,\" -P \",NumberOfProcessors,\" \\\"$maindir/bin/ProteinEvolverProtABC2.0.0\\\"\",sep=\"\")\n";
	print FILE_1 "	write(paste(\"\",bodyhereNew,\"\",sep=\"\"),\"$setsdir/ProteinEvolverABC_Phase3.sh\",append=T)\n";
	}
else
	{
	print FILE_1 "	bodyhereNew<-paste(\"cat \\\"$setsdir/ProteinEvolverABC_arguments.txt\\\" | xargs -n \",NumberOfArguments,\" \\\"$maindir/bin/ProteinEvolverProtABC2.0.0\\\"\",sep=\"\")\n";
	print FILE_1 "	write(paste(\"\",bodyhereNew,\"\",sep=\"\"),\"$setsdir/ProteinEvolverABC_Phase3.sh\",append=T)\n";
	}
