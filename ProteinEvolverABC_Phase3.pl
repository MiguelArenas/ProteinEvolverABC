...
my $SSCPE = 0; # If 0, SSCPE is not run. If 1, SSCPE runs // Here this also works for ProtASR2 -> this is for SS
...
while (<FROM>) 
	...
	if ($_ =~ /^#/)
        ...
        if ($_ =~ /^*ShowInformationScreen=/)
            ...
            # Sequence of the PDB file, required for SSCPE/ProtASR2, for SS
            if ($_ =~ /^SSpdbsequence=/)
                {
                    ($SSSeqPDB_init, $SSSeqPDB_dip) = split(/=/, $_);
                    $SSSeqPDB_dip =~ s/\n//g; # remove the end of line
                    
                    print "  File with the PDB sequence for summary statistics: $SSSeqPDB_dip\n";
                    ## $NumberOfArguments = $NumberOfArguments + 1; # add arguments -@
                        
                    if ($SSPDB_dip ne "default" && $SSSeqPDB_dip eq "default")
                        {
                        print "\n\nERROR!. The calculation of SS based on a PDB protein structure ($SSPDB_dip) requires a file with the PDB sequence ($SSSeqPDB_dip)\n\n";
                        print "Type CTRL+C to abort the execution \n";
                        my $HereError = <STDIN>;
                        chop($HereError);
                        exit;
...
if ($SSPDB_dip ne "default" && $SSSeqPDB_dip ne "default" && $SSChain_dip ne "default") # Doing SSCPE/ProtASR2
    {
    $SSCPE = 1;
    }
...
if (-e "$setsdir/sequences000000001")
    {
    print "\n  \"$setsdir/sequences000000001\" was detected... Ok!, do not simulate again \n";
    }
else # Run the simulations
    {
...
###############
# Obtaining simulations with sequence of the PDB file (given by the user) - if SSCPE/ProtASR2
if ($SSCPE == 1)
    {
    print "\n\n> Obtaining simulations with sequence of the PDB file (given by the user) ... ";
    
    if (-e "$setsdir/sequences000000001_pdb")
        {
        print "\n  \"$setsdir/sequences000000001_pdb\" was detected... Ok!, do not obtain this files again \n";
        }
    else
        {
        system ("perl \"$maindir/scripts/PDBseq_simulated_data.pl\" \"$setsdir/$file\" ");
        }
    }



# Generate ProteinEvolverABC_Phase4.sh and ArgumentsSS.txt for the SS (next step)
print "\n\n> Generating files \"ProteinEvolverABC_Phase4.sh\" and \"ArgumentsSS.txt\" for the phase 4... ";
if ($OperativeSystem eq "linux" && $numberOfProcessors > 1)
	{
	open (OUT_FILE,'>ProteinEvolverABC_Phase4.sh');
    
    # ELENA
        
    if ($SSCPE == 0) # SS from the protein structure are not calculated
        {
            if ($indels == 1)
            {
                print OUT_FILE "cat \"$setsdir/ArgumentsSS.txt\" | xargs -n 3 -P $numberOfProcessors \"$maindir/scripts/Run_ComputeSSfromSimulatedData.sh\" ";
            }
            else
            {
                print OUT_FILE "cat \"$setsdir/ArgumentsSS.txt\" | xargs -n 3 -P $numberOfProcessors \"$maindir/scripts/Run_ComputeSSfromSimulatedData_NoIndels.sh\" ";
            }
        }
        ...
        if ($indels == 1)
            {
            print OUT_FILE "cat \"$setsdir/ArgumentsSS.txt\" | xargs -n 6 -P $numberOfProcessors \"$maindir/scripts/Run_ComputeSSfromSimulatedData_STR.sh\" ";
            }
        else
            {
            print OUT_FILE "cat \"$setsdir/ArgumentsSS.txt\" | xargs -n 6 -P $numberOfProcessors \"$maindir/scripts/Run_ComputeSSfromSimulatedData_NoIndels_STR.sh\" ";
            }
	...
	open (OUT_FILE,'>ProteinEvolverABC_Phase4.sh');
        ...
        if ($SSCPE == 0) # SS from the protein structure are not calculated
        {
            if ($indels == 1)
            {
                print OUT_FILE "cat \"$setsdir/ArgumentsSS.txt\" | xargs -n 3 \"$maindir/scripts/Run_ComputeSSfromSimulatedData.sh\" ";
            }
            else
            {
                print OUT_FILE "cat \"$setsdir/ArgumentsSS.txt\" | xargs -n 3 \"$maindir/scripts/Run_ComputeSSfromSimulatedData_NoIndels.sh\" ";
            }
        }
        else # SS from the protein structure are calculated
        {
            if ($indels == 1)
            {
                print OUT_FILE "cat \"$setsdir/ArgumentsSS.txt\" | xargs -n 6 \"$maindir/scripts/Run_ComputeSSfromSimulatedData_STR.sh\" ";
            }
            else
            {
                print OUT_FILE "cat \"$setsdir/ArgumentsSS.txt\" | xargs -n 6 \"$maindir/scripts/Run_ComputeSSfromSimulatedData_NoIndels_STR.sh\" ";
            }
        }
...
for ($numberSim = 1; $numberSim <= $NumberSimulations; $numberSim++)
    ...
    if ($SSCPE == 1) # SS from the protein structure are calculated
        {
        if ($numberSim < 10)
            {
            print OUT_FILE_2 "$setsdir/sequences00000000$numberSim $save_simulations $show_info $SSPDB_dip $SSChain_dip $SSSeqPDB_dip\n";
            }
        if ($numberSim > 9 && $numberSim < 100)
            {
            print OUT_FILE_2 "$setsdir/sequences0000000$numberSim $save_simulations $show_info $SSPDB_dip $SSChain_dip $SSSeqPDB_dip\n";
            }
        if ($numberSim > 99 && $numberSim < 1000)
            {
            print OUT_FILE_2 "$setsdir/sequences000000$numberSim $save_simulations $show_info $SSPDB_dip $SSChain_dip $SSSeqPDB_dip\n";
            }
        if ($numberSim > 999 && $numberSim < 10000)
            {
            print OUT_FILE_2 "$setsdir/sequences00000$numberSim $save_simulations $show_info $SSPDB_dip $SSChain_dip $SSSeqPDB_dip\n";
            }
        if ($numberSim > 9999 && $numberSim < 100000)
            {
            print OUT_FILE_2 "$setsdir/sequences0000$numberSim $save_simulations $show_info $SSPDB_dip $SSChain_dip $SSSeqPDB_dip\n";
            }
        if ($numberSim > 99999 && $numberSim < 1000000)
            {
            print OUT_FILE_2 "$setsdir/sequences000$numberSim $save_simulations $show_info $SSPDB_dip $SSChain_dip $SSSeqPDB_dip\n";
            }
        if ($numberSim > 999999 && $numberSim < 10000000)
            {
            print OUT_FILE_2 "$setsdir/sequences00$numberSim $save_simulations $show_info $SSPDB_dip $SSChain_dip $SSSeqPDB_dip\n";
            }
        if ($numberSim > 9999999 && $numberSim < 100000000)
            {
            print OUT_FILE_2 "$setsdir/sequences0$numberSim $save_simulations $show_info $SSPDB_dip $SSChain_dip $SSSeqPDB_dip\n";
            }
        if ($numberSim > 99999999 && $numberSim < 1000000000)
            {
            print OUT_FILE_2 "$setsdir/sequences$numberSim $save_simulations $show_info $SSPDB_dip $SSChain_dip $SSSeqPDB_dip\n";
            }
        if ($numberSim > 999999999)
            {
            print "Warning!. Too many simulations, check ProteinEvolverABC_Phase3.pl\n";
            print "Type CTRL+C to abort the execution \n\n";
            my $error = <STDIN>;
            chop($error);
            }
        }
    else # SS from the protein structure are not calculated
        {
            if ($numberSim < 10)
                {
...
if ($SSCPE == 1)
    ...
    if (-e "UserEAAMsites")
        {
        system ("mv UserEAAMsites \"$setsdir/OtherOutputs\"");
        }
    #system ("mv UserEAAMsites \"$setsdir/OtherOutputs\"");
    }
